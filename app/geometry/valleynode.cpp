...
ValleyNode::ValleyNode(QVector<QVector2D> uvs) : BaseNode("river"), uv(uvs), deposit(NULL)
{
    smooth(uv);
...
QVector<Vector3> ValleyNode::intersectionPoints(Vector3 from, Vector3 direction) {
//    if (active) {
...
void ValleyNode::determineActionOnStoppedDrawing() {
...
void ValleyNode::addPoint(Vector3 from, Vector3 direction) {
...
void ValleyNode::doTransformSurface(QVector < QVector < Vector3 > > & rows, float resolution, float) {
...
void ValleyNode::smooth(QVector<QVector2D>& uv) {
    if (uv.size()<1) return;
...
void ValleyNode::removeLoops(QVector<QVector2D>& sp) {
...
void ValleyNode::repositionOnSurface(SurfaceNode &surfacenode) {
    rightSpline.clear();
    spline.clear();
...
void ValleyNode::addSubclassJson(QVariantMap &map) {
...
void ValleyNode::fromJsonSubclass(QVariantMap map) {
    rigths = variantToVector2DVector(map["rights"]);
    lefts = variantToVector2DVector(map["lefts"]);
}

void ValleyNode::createDeposit(float seaLevel, SurfaceNode& surfaceNode) {
    ...
    for (int i = 0; i < lefts.size()-1; i++) {
        ...
        if (point.y() < seaLevel) {
            //if (i == 0) return;
            Deposit * deposit = new Deposit(lefts[i], lefts[i] - lefts[i-1], seaLevel, &surfaceNode);
